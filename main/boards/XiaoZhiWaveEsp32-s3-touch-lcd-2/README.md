...
- ESP32-S3-Touch-LCD-2 
    ...
    - 20脚 连接 功放BCLK
    - 19脚 连接 功放DIN
    - 功放 GAIN 悬空，或按 MAX98357 数据手册用电阻设置增益
    - 功放 SD 接 3.3V 常开，不要连接开发板 17/18 脚
    - 3脚  连接 4G模块RXD
    - 4脚  连接 4G模块TXD
...
- 固件默认使用 ML307R 4G 网络。
- ESP32-S3 的 GPIO43（开发板 3 脚 TXD）连接 ML307R RXD。
- ESP32-S3 的 GPIO44（开发板 4 脚 RXD）连接 ML307R TXD。
- ML307R 与开发板必须共地；模块使用 VIN 供电时不要同时给 BAT 引脚供电。
- 在启动、配网或空闲状态长按 BOOT 键，可在 4G 与 Wi-Fi 之间切换；切换后设备自动重启。
- Wi-Fi 模式下，启动或配网阶段单击 BOOT 键可进入 Wi-Fi 配网。
- GPIO43/GPIO44 已专用于 4G UART，固件日志通过 USB Serial/JTAG 输出。
- ML307R UART 会自动协商后切换到模块支持的最高档 `921600` bps。
